...
if (modal && modalClose) {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape") { closeModal(); closeCaseStudy(); }
  });
}
...
(function () {
  var CASE_STUDIES = {
    ...
    lentera: {
      ...
      solution:"Mesin dongeng interaktif dengan 17 cerita rakyat Nusantara, karakter SVG animasi original, background scene dinamis, ambient sound procedural, dan sistem progress berbintang untuk memotivasi anak terus membaca.",
      result:"Platform yang dapat dinikmati anak usia 4–10 tahun tanpa koneksi internet, dengan 17 cerita lengkap dengan pesan moral, sistem bintang yang mendorong engagement, dan pengalaman audio-visual yang imersif.",
      features:["📚 17 Dongeng Nusantara","🎭 Karakter SVG Animasi Original","🔊 Ambient Sound Engine","⭐ Sistem Progress & Reward","🔍 Filter & Pencarian Cerita","💾 Offline-capable"],
  ...
  function openCaseStudy(key) {
    ...
    if (body) {
      ...
      body.addEventListener("scroll", function onScroll(){
        if (body.scrollTop > 40){ if(hint) hint.classList.add("cs-hint-hidden"); body.removeEventListener("scroll",onScroll); }
      },{ passive:true });
    }
  }
  function closeCaseStudy() {
    csPanel.classList.remove("cs-panel-in");
    setTimeout(function(){ csModal.classList.remove("cs-open"); document.body.style.overflow=""; },350);
  }
  window.closeCaseStudy = closeCaseStudy;
  document.querySelectorAll(".cs-open-btn").forEach(function(btn){
    btn.addEventListener("click",function(e){ e.stopPropagation(); openCaseStudy(btn.dataset.project); });
  });
...
(function () {
  ...
  function resize(){
    ...
    var wrap=canvas.parentElement; W=wrap.clientWidth||400; H=wrap.clientHeight||200;
    canvas.width=W*dpr; canvas.height=H*dpr; canvas.style.width=W+"px"; canvas.style.height=H+"px";
    ctx.scale(dpr,dpr); fillBg();
  }
  function noise(x,y){
    ...
    var g=function(h,px,py){ h=h&3; var gx=h<2?(h===0?1:-1):0; var gy=h>=2?(h===2?1:-1):0; return gx*px+gy*py; };
    return (1-u)*(1-v)*g(h1,xf,yf)+u*(1-v)*g(h2,xf-1,yf)+(1-u)*v*g(h3,xf,yf-1)+u*v*g(h4,xf-1,yf-1);
  }
  function Particle(){ this.reset(); }
  Particle.prototype.reset=function(){
    this.x=Math.random()*W; this.y=Math.random()*H; this.vx=0; this.vy=0;
  ...
  function loop(ts){
    ...
    ctx.globalCompositeOperation="source-over"; ctx.fillStyle="rgba(5,5,8,0.18)"; ctx.fillRect(0,0,W,H);
    ctx.globalCompositeOperation="lighter";
    var cx=W/2, cy=H/2;
    for(var i=0;i<particles.length;i++){
      var p=particles[i]; p.orbitA+=p.orbitSpd*dt;
      ...
      p.vx+=(tx-p.x)*0.016*dt+Math.cos(flowA)*0.25; p.vy+=(ty-p.y)*0.016*dt+Math.sin(flowA)*0.25;
      p.vx*=0.93; p.vy*=0.93;
      ...
      if(p.trail.length>1){
        ...
        ctx.strokeStyle=p.color; ctx.globalAlpha=p.alpha*0.45; ctx.lineWidth=p.r*0.7; ctx.lineCap="round"; ctx.stroke();
      }
      ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle=p.color; ctx.globalAlpha=p.alpha; ctx.fill(); ctx.globalAlpha=1;
    }
  }
  (function earlyInit(){
    dpr=Math.min(window.devicePixelRatio||1,2); var wrap=canvas.parentElement;
    if(wrap&&wrap.clientWidth>0){ W=wrap.clientWidth; H=wrap.clientHeight||220; canvas.width=W*dpr; canvas.height=H*dpr; canvas.style.width=W+"px"; canvas.style.height=H+"px"; ctx.scale(dpr,dpr); fillBg(); }
  })();
  if("IntersectionObserver" in window){
    var obs=new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if(e.isIntersecting&&!started){ started=true; init(); rafId=requestAnimationFrame(loop); }
        else if(!e.isIntersecting&&started){ cancelAnimationFrame(rafId); rafId=null; started=false; }
      });
    },{threshold:0.05});
    obs.observe(canvas);
  } else { init(); rafId=requestAnimationFrame(loop); }
  window.addEventListener("resize",function(){
    if(started&&rafId){ cancelAnimationFrame(rafId); init(); rafId=requestAnimationFrame(loop); } else { fillBg(); }
  },{passive:true});
})();
